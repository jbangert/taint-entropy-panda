...
print "char icls[65536];"
print "void init_icls(){"
print "memset(&icls,ICLS_WARN, sizeof icls);"
output = subprocess.check_output(["tr '\t ' '\n' | grep 'I_'"],stdin=sys.stdin, shell=True,universal_newlines=True)

for dist in output.split('\n'):
    if not("I_" in dist):
        continue
    ins = dist.replace('I_','').lower()
    print "icls["+dist+"] = ",
    if not (ins in mnem2type):
        print "ICLS_WARN;"
        continue
    g = mnem2type[ins]
    if 'mov' in ins:
        print "ICLS_MOV;"
    elif 'logical' in g or 'shftrot' in g or 'bit' in g or 'binary' in g or 'arith' in g:
        print "ICLS_BITARITH;"
    else:
        print "ICLS_NORMAL;"
print "}"
